//
//  BespinSimpleAlert.swift
//  BespinApp
//
//  Created by DJ McKay on 11/29/18.
//

import UIKit


public class BespinSimpleAlert {
    
    //==========================================================================================================
    // Feel free to edit these variables
    //==========================================================================================================
    public struct Settings {
        public static var BackgroundColor = UIColor(red: 255/255, green: 75/255, blue: 56/255, alpha: 0.66)
        ...
        public static var FailColor = UIColor(red: 227/255, green: 232/255, blue: 235/255, alpha: 1.0)
        public static var ActivityWidth = UIScreen.ScreenWidth / Settings.WidthDivision
        public static var ActivityHeight = ActivityWidth / 2
        ...
        public static var DarkensBackground = true
    }
    ...
    /// Disable UI stops users touch actions until BepsinActivityIndicator is hidden. Return success status
    @discardableResult
    public static func show(_ text: String, disableUI: Bool) -> Bool {
        guard instance == nil else {
            print("BepsinActivityIndicator: You still have an active activity, please stop that before creating a new one")
            return false
        }
        
        guard topMostController != nil else {
            print("BepsinActivityIndicator Error: You don't have any views set. You may be calling them in viewDidLoad. Try viewDidAppear instead.")
            return false
        }
    ...
    @objc public static func dismiss() {
        guard instance != nil else {
            print("BepsinActivityIndicator: You don't have an activity instance")
            return
        }
        
        guard hidingInProgress == false else {
            print("BepsinActivityIndicator: Hiding already in progress")
            return 
        }
        
        if !Thread.current.isMainThread {
            DispatchQueue.main.async {
                instance?.hideLoadingActivity(true, animated: true )
            }
        } else {
            instance?.hideLoadingActivity(true, animated: true)
        }
...
//            activityView.startAnimating()
            ...
            dismissButton.frame = CGRect(x: 10, y: yPosition, width: 40, height: 40)
            dismissButton.backgroundColor = UIColor.clear
            dismissButton.setTitle(Settings.FailIcon, for: .normal)
            dismissButton.addTarget(BespinSimpleAlert.self, action: #selector(dismiss), for: .touchUpInside)
            textLabel = UILabel(frame: CGRect(x: 60, y: 10, width: Settings.ActivityWidth - 70, height: Settings.ActivityHeight - 10))
